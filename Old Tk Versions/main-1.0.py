...
# Read in the datafile, using the pkl file if needed
name, extention = os.path.splitext(FILENAME)
if extention == '.pkl': # First check if we have a pickle file to read
  data = pd.read_pickle(os.path.join(sys.path[0], FILENAME))
elif checkPklFile:  # Next check if we need to see if we need to look for one
  pklFile = name + '.pkl'
  if os.path.exists(pklFile):
    data = pd.read_pickle(os.path.join(sys.path[0], pklFile))
elif not extention == '.csv':
  print('file must be of type csv or pkl')
  sys.exit()

# If we did not read in a pkl file, read in the csv
...
STATS = ['kills', 'deaths', 'assists', 'dpm', 'damageshare', 'csat10', 'xpat10', 'csdiffat10', 'golddiffat10', 'csat15', 'xpat15', 'csdiffat15', 'golddiffat15']
...
class GUI:
  ...
  def deleteTeam(self, i, *args):
    if teamMenus[i] is None or not teamNames[i].get() == 'remove team':
      return
  ...
  def updateTeams(self, *args):
    ...
    for index in range(len(teamMenus)):
      ...
      if team is None or not team.winfo_exists() == 1:
  ...
  def updatePlayers(self, *args):
    ...
    for index in range(len(playerMenus)):
      ...
      if player is None or not player.winfo_exists() == 1:
        continue
      menu = player['menu']
      menu.delete(0, 'end')
      for p in PLAYERS:
